...
function onSalesDirectChange(inp) {
  ...
  ensureShipments(co);
  const pid    = prod.replace(/[^a-zA-Z0-9]/g,'_');
  // Ambiguous text (e.g. "2.000") — fmtThousandInline already flagged the
  // field. Show why and lock Simpan; never fall through to parseFloat, which
  // is what used to turn 2000 into 2.
  if (inp.dataset.mtAmbiguous) {
    const errAmb = g(`util-err-${pid}-${idx}`);
    if (errAmb) {
      errAmb.textContent = 'Format tidak jelas — pakai koma untuk ribuan (2,000), bukan titik.';
      errAmb.style.display = 'block';
      errAmb.style.color   = 'var(--red2)';
      errAmb.classList.add('show');
    }
    const btnAmb = g(`util-save-${pid}-${idx}`);
...
function saveSalesUtil(prod, idx) {
  ...
  // parseMT returns null for empty, malformed OR ambiguous text ("2.000").
  // Treating null as 0 here would re-open the silent-corruption path, so an
  // ambiguous field stops the save outright.
  const newMT = directInp.value.trim() === '' ? 0 : parseMT(directInp.value);
  if (newMT === null) {
    const errEl0 = g(`util-err-${pid}-${idx}`);
    if (errEl0) {
      errEl0.textContent = 'Format tidak jelas — pakai koma untuk ribuan (2,000), bukan titik.';
      errEl0.style.display = 'block';
      errEl0.style.color   = 'var(--red2)';
      errEl0.classList.add('show');
    }
    directInp.focus();
    return;
  }
  ...
  directInp.value = newMT > 0 ? newMT.toLocaleString(MT_LOCALE) : '';
...
function onOpsRealChange(inp) {
  ...
  ensureShipments(co);
  // Ambiguous text is flagged by fmtThousandInline — leave the model untouched
  // rather than writing a guessed realMT. saveEdit() blocks the save too.
  if (inp.dataset.mtAmbiguous) return;
  const newVal = inp.value.trim() === '' ? null : parseMT(inp.value);
...
function collectShipmentData(co) {
  ...
  document.querySelectorAll('.sales-util-direct-inp').forEach(inp => {
    ...
    if (co.shipments && co.shipments[prod] && co.shipments[prod][idx]) {
      // parseMT rejects ambiguous text; skip the field instead of storing a
      // guess. saveEdit() refuses the whole save when any field is flagged, so
      // reaching here with one is not expected — this is belt-and-braces.
  ...
  document.querySelectorAll('.ops-real-inp').forEach(inp => {
    ...
    const idx  = parseInt(inp.dataset.idx);
    if (co.shipments && co.shipments[prod] && co.shipments[prod][idx]) {
      const val = inp.value.trim() === '' ? null : parseMT(inp.value);
      if (val === null && inp.value.trim() !== '') return;   // ambiguous — leave as-is
      co.shipments[prod][idx].realMT  = val;
      co.shipments[prod][idx].arrived = val != null && val > 0;
    }
  });
...
function buildRevisionRequestTable(co) {
  ...
  function buildTargetRows(sourceProd, targets, disabled) {
    ...
    return targets.map((t, i) => {
      ...
      const mt = t.mt != null ? Number(t.mt).toLocaleString(MT_LOCALE) : '';
