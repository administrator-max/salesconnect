...
function renderMain() {
    ...
    if (isMulti) {
      ...
      d.subRows.forEach(s => {
        ...
        trSub.innerHTML = `
          ...
          <td class="t-r">${s.realP>0?`<span style="color:var(--green);font-family:'DM Mono',monospace">${fmtMt(Number(s.realP))}</span>`:'<span style="color:var(--txt3)">—</span>'}</td>
  ...
  if (tfoot) {
    ...
    tfoot.innerHTML = `<tr class="tr-totals">
      ...
      <td class="t-r t-mono" style="color:var(--green)">${fmtMt(tReal)}</td>
  ...
  if (totBar) {
    ...
    totBar.innerHTML =
      ...
      `<span style="opacity:.3">·</span>` +
      `<span style="color:var(--green)">Realized: <strong>${fmtMt(tR)}</strong></span>` +
      `<span style="opacity:.3">·</span>` +
